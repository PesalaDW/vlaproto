...
def move_kuka_to(position, kuka_id):
    end_effector_index = 6  # Usually link 6 for KUKA IIWA
    joint_angles = p.calculateInverseKinematics(kuka_id, end_effector_index, position)
...
object_info = []  # NEW: to store shape, color, body_id, position

for shape, color, pos in zip(shapes, list(colors.keys()) * 2, positions):
    obj_id = create_object(shape, colors[color], pos)
    object_info.append({
        "shape": shape,
        "color": color,
        "id": obj_id,
...
with open("action_command.json", "r") as f:
    action = json.load(f)
...
if target_position:
    print(f"Target object found at: {target_position}")
    target_position[2] += 0.1  # Move above the object
else:
    print("Target object not found.")

for _ in range(240 * 2):  # Wait 2 seconds
    p.stepSimulation()
    time.sleep(1 / 240)

if target_position:
    move_kuka_to(target_position, kuka_id)
...
301: Camera distance in line 104 changed.
310: Calling action.
    In fact, major changes were made. So I'll go 4xx.
400: 310 renamed.
    40x:
        401: Setting a small time delay.
        402: Bypassing top down view.
"""
